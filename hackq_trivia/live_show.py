...
class LiveShow:
    ...
    async def handle_msg(self, message: Dict) -> None:
        ...
        if message_type == 'broadcastEnded':
            ...
            hook.send(f'{ans_str}:{answer["count"]}:{answer["correct"]}\n\n{message["advancingPlayersCount"]} players advancing\n\n{message["eliminatedPlayersCount"]} players eliminated\n')
